...
class Details_products extends Controller
{
    public function getdaitails(Request $request){
        ...
        $idcat=product::join('categories','categories.id','=','products.id_category')
        ->where('products.id',$id)
        ->select('categories.id as id','categories.name as name')
        ->first();
        
        $relatedproduct=product::
        where('id_category',$idcat->id)
        ->select('products.name as name','products.id as id','products.ratting as ratting','products.price as price','products.description as desc','products.Caracteristique as carq','products.new_price as newprice','products.image as img','products.qte as qte')
        ->get();
        return view('details',compact('categorie','prductsdtailsproduct','listimages','products_cards','total_price','firstencategorie','lastcategories','idcat','relatedproduct'));
    }
}
